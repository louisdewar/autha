...
use self::error::{GetLimiterPermitError, RateLimitReached};
...
/// Another config may have two strategies (i.e. two `TokenBucket`s) such that there is a limit of 10
...
impl LimiterConfig {
    /// The limiter ID must be unique across all rate limiters in the same redis DB.
    /// The ID is used to generate the keys.
